    ...
    <div class="accordion-filter">
      <ul>
        <li v-for="item in itemsFormat" class="{{ item.className }}"
          ...
          <a v-if="item.haveChild===true" href="#" tabindex="1"
    ...
    methods: {
      itemOnClick: function (event) {
        ...
        if (role === 'li' || role === 'li-child') { // 没有子节点
          ...
          // 如果是子节点 要更新父节点状态
          if (role === 'li-child') {
            var parentLi = event.currentTarget.parentNode.parentNode;
            var pid = parentLi.firstElementChild.getAttribute('idx');
            var childLis = event.currentTarget.parentNode.children;
            var checkCount = 0;
            if (childLis.length === 0) {
              return;
            }
            for (var i = 0; i < childLis.length; i++) {
              if (childLis[i].firstElementChild.checked) {
                checkCount++;
              }
            }
            if (checkCount === childLis.length) { // 全部选中
              parentLi.setAttribute('data-state', ALL_SELECTED);
              this.setItemChecked(pid, true);
            } else if (checkCount < childLis.length && checkCount > 0) {
              parentLi.setAttribute('data-state', SOME_SELECTED);
              this.setItemChecked(pid, false);
            } else if (checkCount < childLis.length && checkCount === 0) {
              parentLi.setAttribute('data-state', NON_SELECTED);
              this.setItemChecked(pid, false);
            }
          }
        } else if (role === 'link-hc') { // 有子节点
          var isCheckeds = false;
          var childNodes = event.currentTarget.parentNode.children || [];
          var childUl = null;
          var childLi = [];
          ...
          if (selectState === ALL_SELECTED) {
            ...
            // 对父节点的checked状态做变更
            this.setItemChecked(idx, isCheckeds);
          ...
          if (childNodes.length > 0) {
            ...
            if (childUl.tagName === 'UL') {
              ...
              for (var index = 0; index < childLi.length; index++) {
                var liNode = childLi[index];
