...
var selectDropDown = Vue.component('taurus-select-dropdown', {
    ...
    echoSelectedValue: function (currentValue) {
      for (var i = 0; i < this.list.length; i++) {
        ...
        if (currentValue === listItem['value']) {
          ...
          // $set方法设置激活项实时更新DOM
          this.list.$set(i, this._deepClone(listItem));
        } else {
    ...
    _processResponseData: function (data) {
      ...
      if (data.length > 0) {
        for (var i = 0; i < data.length; i++) {
          ...
          if (Number(data[i][this.rename.value]) === Number(this.defaultvalue)) {
            tempObj['active'] = true;
          } else {
  ...
  watch: {
    ...
    bmValue: function (newValue, oldValue) {
      var selectedObj = this._setSelectedData(newValue);
      this.selectedValue = newValue;
      if (newValue) {
        this.selectedLabel = selectedObj.label;
      } else {
        this.selectedLabel = DEFAULT_PLACEHOLDER;
      }
    }
  }
});
